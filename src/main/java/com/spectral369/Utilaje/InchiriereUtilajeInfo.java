...
public class InchiriereUtilajeInfo extends HorizontalLayout implements RouterLayout, AfterNavigationObserver {
    ...
    public InchiriereUtilajeInfo() {
		...
		IarbaTitle = new Button("Inchiriere Utilaje", VaadinIcon.FILE_TEXT_O.create());
		...
		generate.addClickListener(evt -> {
			...
			PDFUtilajeCreator pdfcr =  new PDFUtilajeCreator(map,Utils.getTimeStr());
		 	String fn =  pdfcr.getID();
		    
			 RouteConfiguration.forSessionScope().removeRoute(InchiriereUtilajePDF.class);
		 	 RouteConfiguration.forSessionScope().setRoute(InchiriereUtilajePDF.NAME, InchiriereUtilajePDF.class);
		Map <String,String> sss =  new HashMap<String,String>();
		sss.put("tm", fn);
		
		UI.getCurrent().navigate("InchiriereUtilajePDF",QueryParameters.simple(sss));
		});
